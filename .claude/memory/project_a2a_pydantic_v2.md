...
- The structured application result is a data artifact containing exactly the
  duplicated `MonitoringResponse` schema. Keep the backend and agent copies in
  sync.
...
- The temporary A2A v0.3-compatible JSON-RPC route is marked for removal after
  one stable release window. New code must use 1.x APIs only.
