...
from app.services.ai_integrations.perplexity_client import PerplexityClient, PERPLEXITY_API_URL
from app.core.config import Settings

@pytest.fixture
def mock_settings_perplexity(): # Renamed fixture
    with patch('app.services.ai_integrations.perplexity_client.settings') as mock_settings_obj:
...
@pytest.mark.asyncio
@patch('app.services.ai_integrations.perplexity_client.PerplexityClient._make_perplexity_request')
async def test_refine_query_success(mock_make_request, mock_settings_perplexity):
    ...
    mock_make_request.return_value = {"choices": [{"message": {"content": expected_refined}}]}
    ...
    called_payload = mock_make_request.call_args[0][0] # First positional arg to the mock
...
@pytest.mark.asyncio
@patch('app.services.ai_integrations.perplexity_client.PerplexityClient._make_perplexity_request')
async def test_refine_query_malformed_response(mock_make_request, mock_settings_perplexity):
...
async def test_identify_sources_success(mock_make_request, mock_settings_perplexity):
    ...
    mock_make_request.return_value = {"choices": [{"message": {"content": mock_response_content_string}}]}
...
@pytest.mark.asyncio
@patch('app.services.ai_integrations.perplexity_client.PerplexityClient._make_perplexity_request')
async def test_identify_sources_empty_response(mock_make_request, mock_settings_perplexity):
...
@pytest.mark.asyncio
@patch('app.services.ai_integrations.perplexity_client.PerplexityClient._make_perplexity_request')
async def test_identify_sources_malformed_response(mock_make_request, mock_settings_perplexity):
    client = PerplexityClient(api_key="test_key_identify_malformed")
    refined_query = "Malformed response query"
    mock_make_request.return_value = {"invalid": "structure"}

    with pytest.raises(ValueError, match="Invalid response structure from Perplexity API for identify_sources"):
