...
def mock_settings_openai():  # Renamed to avoid conflict if conftest has similar
    with patch(
        "app.services.ai_integrations.openai_client.settings"
...
@pytest.mark.asyncio
@patch("app.services.ai_integrations.openai_client.OpenAIClient._make_openai_request")
async def test_generate_embeddings_malformed_response(
    mock_make_request, mock_settings_openai
):
    ...
    with pytest.raises(
        ValueError,
        match="Invalid response structure from OpenAI API for generate_embeddings",
    ):
        await client.generate_embeddings(texts)
...
async def test_analyze_diff_success_json(mock_make_request, mock_settings_openai):
    ...
    mock_make_request.return_value = {
        "choices": [{"message": {"content": mock_content_string}}]
    }

    analysis = await client.analyze_diff(
        old_rep, new_rep, api_params={"response_format": {"type": "json_object"}}
    )
...
@pytest.mark.asyncio
@patch("app.services.ai_integrations.openai_client.OpenAIClient._make_openai_request")
async def test_analyze_diff_invalid_json_response(
    mock_make_request, mock_settings_openai
):
    ...
    mock_make_request.return_value = {
        "choices": [{"message": {"content": invalid_json_content}}]
    }
