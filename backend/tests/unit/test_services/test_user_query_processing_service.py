...
@pytest.mark.asyncio
async def test_process_query_success(mock_db_session: AsyncMock, mock_discovery_service: AsyncMock):
    query_id = 1
    raw_query_text = "test query"
    discovered_urls = ["http://example.com/1", "http://example.com/2"]

    mock_user_query_instance = UserQuery(id=query_id, raw_query=raw_query_text, status="pending_discovery")
    
    mock_result_user_query = AsyncMock()
    mock_result_user_query.scalar_one_or_none = MagicMock(return_value=mock_user_query_instance)
    ...
    async def execute_side_effect(statement, *args, **kwargs):
        compiled_stmt = str(statement.compile(compile_kwargs={"literal_binds": True})).lower()
        if "user_queries" in compiled_stmt and f"user_queries.id = {query_id}" in compiled_stmt:
            return mock_result_user_query
        elif "monitored_sources" in compiled_stmt and f"monitored_sources.user_query_id = {query_id}" in compiled_stmt:
    ...
    await processing_service.process_query(query_id, mock_db_session, mock_discovery_service)
    ...
    assert any(isinstance(obj, UserQuery) and obj.status == "processed" for obj in added_objects)
    assert sum(1 for obj in added_objects if isinstance(obj, MonitoredSource)) == len(discovered_urls)

@pytest.mark.asyncio
async def test_process_query_no_query_found(mock_db_session: AsyncMock, mock_discovery_service: AsyncMock, processing_service: UserQueryProcessingService):
    query_id = 99
    mock_result_user_query = AsyncMock()
...
@pytest.mark.asyncio
async def test_process_query_already_processed(mock_db_session: AsyncMock, mock_discovery_service: AsyncMock, processing_service: UserQueryProcessingService):
    query_id = 2
    raw_query_text = "already processed query"
    mock_user_query_instance = UserQuery(id=query_id, raw_query=raw_query_text, status="processed")
...
@pytest.mark.asyncio
async def test_process_query_discovery_fails(mock_db_session: AsyncMock, mock_discovery_service: AsyncMock, processing_service: UserQueryProcessingService):
    query_id = 3
    ...
    mock_result_error_fetch.scalar_one_or_none = MagicMock(return_value=mock_user_query_instance)

    async def execute_side_effect_fail(statement, *args, **kwargs):
        compiled_stmt = str(statement.compile(compile_kwargs={"literal_binds": True})).lower()
        if "user_queries" in compiled_stmt and f"user_queries.id = {query_id}" in compiled_stmt and mock_discovery_service.discover_sources.call_count == 0:
            return mock_result_user_query
        elif "user_queries" in compiled_stmt and f"user_queries.id = {query_id}" in compiled_stmt and mock_discovery_service.discover_sources.call_count > 0:
    ...
    mock_discovery_service.discover_sources = AsyncMock(side_effect=Exception("Discovery API down"))
