import pytest
from sqlalchemy import create_engine # Keep for engine definition if needed locally, but maybe not
from sqlalchemy.orm import sessionmaker # Keep for TestingSessionLocal if needed locally
...
@pytest.mark.asyncio
async def test_user_query_creation(db_session: AsyncSession): # Mark async, use async db_session
...
async def test_user_query_update_timestamp(db_session: AsyncSession): # Mark async
    ...
    async with db_session.begin(): # New transaction or re-use with begin for fetching
        ...
        assert updated_query.created_at == initial_created_at # Created_at should not change
