...
from sqlalchemy.orm import Session # Keep Session for type hint if needed, but we use AsyncSession
...
@patch('app.services.source_discovery_service.SourceDiscoveryService.discover_sources', new_callable=AsyncMock) # Mock discovery
async def test_create_user_query_success(
    ...
    mock_discover_sources.return_value = ["http://discovered.com/1", "http://discovered.com/2"]
    ...
    assert task_func.__qualname__.startswith("UserQueryProcessingService.") # More specific check
    ...
    assert db_obj_after_processing.status == "processed" # Status should be processed now
