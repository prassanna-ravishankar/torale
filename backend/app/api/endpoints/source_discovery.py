...
async def discover_sources_endpoint(
    ...
    logger.info(
        f"Received request to discover sources for query: '{query_input.raw_query[:50]}...'"
    )
    """
    ...
    try:
        ...
        logger.info(
            f"Successfully discovered {len(monitorable_urls)} sources for query '{query_input.raw_query[:50]}...'"
        )
        ...
        if (
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid input or parameters: {e}"
            )
        ...
        raise HTTPException(
            status_code=501, detail=f"AI functionality not implemented: {e}"
        )
    except Exception as e:
        logger.exception(
            f"Unexpected error in discover_sources_endpoint for query '{query_input.raw_query[:50]}...'"
        )  # Use logger.exception to include stack trace
        raise HTTPException(
            status_code=500, detail="Internal server error during source discovery."
        )
