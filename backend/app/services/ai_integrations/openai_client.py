...
DEFAULT_EMBEDDING_MODEL = "text-embedding-ada-002" # Or a newer model like text-embedding-3-small
...
class OpenAIClient(AIModelInterface):
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            logger.critical("OpenAI API key is required but not found. Set OPENAI_API_KEY in environment.")
        ...
        self.model_name = "openai" # Generic identifier, specific models used in methods
        self.headers = {
    ...
    async def _make_openai_request(self, url: str, payload: Dict) -> Dict:
        async with aiohttp.ClientSession(headers=self.headers) as session:
            try:
                logger.debug(f"Sending request to OpenAI API: {url} with payload: {json.dumps(payload)[:200]}...")
                response = await session.post(url, json=payload) # Await first
                response.raise_for_status() # Then check
                return await response.json()
            except aiohttp.ClientError as e:
                logger.error(f"AIOHTTP client error with OpenAI API ({url}): {e}", exc_info=True)
                raise ConnectionError(f"Failed to connect to OpenAI API: {e}") from e
            except json.JSONDecodeError as e:
                logger.error(f"JSON decode error from OpenAI API ({url}): {e}. Status: {response.status if 'response' in locals() else 'N/A'}", exc_info=True)
                raise ValueError(f"Failed to parse JSON response from OpenAI API: {e}") from e

    async def refine_query(self, raw_query: str, **kwargs) -> str:
        model_to_use = kwargs.get("model", DEFAULT_CHAT_MODEL)
        logger.warning(f"OpenAIClient.refine_query called for '{raw_query[:50]}...' with model {model_to_use} but is not primary. kwargs: {kwargs}")
        raise NotImplementedError("OpenAIClient.refine_query is not the primary task for this client as per current plan.")

    async def identify_sources(self, refined_query: str, **kwargs) -> List[str]:
        logger.warning(f"OpenAIClient.identify_sources called for '{refined_query[:50]}...' but is not suitable. kwargs: {kwargs}")
        raise NotImplementedError("OpenAIClient.identify_sources is not suitable for this client.")
    ...
    async def generate_embeddings(self, texts: List[str], **kwargs) -> List[List[float]]:
        ...
        payload = {
            "input": texts,
            "model": model_to_use,
            **kwargs.get("api_params", {}) # For other params like encoding_format, dimensions
        }
        
        logger.info(f"Generating {len(texts)} embeddings with OpenAI model: {model_to_use}.")
        response_data = await self._make_openai_request(OPENAI_API_URL_EMBEDDINGS, payload)
        
        try:
            embeddings = [item["embedding"] for item in response_data["data"]]
            if not embeddings or len(embeddings) != len(texts):
                logger.error(f"Mismatch in number of embeddings returned ({len(embeddings)}) vs texts ({len(texts)}) from OpenAI or empty list.")
                raise ValueError("Mismatch in number of embeddings returned or empty embeddings list.")
            logger.info(f"Successfully generated {len(embeddings)} embeddings.")
            return embeddings
        except (KeyError, IndexError, TypeError) as e:
            logger.error(f"Error parsing embeddings from OpenAI response. Response: {response_data}", exc_info=True)
            raise ValueError("Invalid response structure from OpenAI API for generate_embeddings.") from e
    ...
    async def analyze_diff(self, old_representation: Any, new_representation: Any, **kwargs) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error parsing diff analysis from OpenAI response. Response: {response_data}", exc_info=True)
            raise ValueError("Invalid response structure from OpenAI API for analyze_diff.") from e 