    ...
    async def detect_changes_for_source(
        ...
        try:
            similarity = self._calculate_similarity(
                current_embedding.embedding_vector, previous_embedding.embedding_vector
            )
        except Exception as e:
            logger.error(
        ...
        if similarity < similarity_threshold:
            ...
            if self.ai_model and hasattr(self.ai_model, "analyze_diff"):
                logger.info(
                    f"Attempting AI diff analysis for source ID {monitored_source_id}"
                )
                try:
                    old_text = (
                        previous_embedding.scraped_content.processed_text
                        or previous_embedding.scraped_content.raw_content
                    )
                    new_text = (
                        current_embedding.scraped_content.processed_text
                        or current_embedding.scraped_content.raw_content
                    )

                    diff_analysis = await self.ai_model.analyze_diff(
                        old_representation=old_text, new_representation=new_text
                    )
            ...
            logger.info(
                f"No significant change detected for source ID {monitored_source_id}"
            )
            return None
