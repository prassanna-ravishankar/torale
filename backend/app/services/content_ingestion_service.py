...
from app.schemas.content_schemas import ScrapedContentSchema, ContentEmbeddingSchema # For data validation if needed
...
class ContentIngestionService:
    ...
    def __init__(self, ai_model: AIModelInterface, db: Session):
        ...
        self.db = db
        logger.info(f"ContentIngestionService initialized with AI model: {getattr(ai_model, 'model_name', type(ai_model).__name__)}")
        if not robotexclusionrulesparser:
            logger.info("robotexclusionrulesparser not installed. Robots.txt checking will be skipped for this service instance.")
            ...
            async with aiohttp.ClientSession(headers={"User-Agent": self._USER_AGENT}) as session:
                async with session.get(robots_url, timeout=5) as response:
                    if response.status == 200:
                        ...
                        logger.info(f"Successfully fetched and parsed robots.txt for {parsed_url.netloc}")
                        return parser
                    else:
                        # No robots.txt or not accessible, assume allow (or cache a specific "allow all" state)
                        logger.warning(f"robots.txt not found or not accessible (status: {response.status}) for {parsed_url.netloc}. Assuming allow.")
                        self._robot_parsers_cache[robots_url] = None # Mark as tried
                        return None 
        except Exception as e:
            logger.error(f"Error fetching or parsing robots.txt for {robots_url}: {e}", exc_info=True)
            self._robot_parsers_cache[robots_url] = None # Mark as tried
            return None
    ...
    def _preprocess_text(self, text: str) -> str:
        ...
        text = BeautifulSoup(text, "html.parser").get_text(separator='\n', strip=False) # Ensure text is clean from HTML entities if any residual
        text = re.sub(r'[\t ]+', ' ', text)  # Replace multiple spaces/tabs with a single space
        text = re.sub(r'\n[\n\s]*\n', '\n\n', text)  # Replace multiple newlines (with optional space in between) with double newlines
    ...
    async def _scrape_website_content(self, url: str) -> Optional[str]:
        ...
        async with aiohttp.ClientSession(headers={"User-Agent": self._USER_AGENT}) as session:
            try:
                ...
                async with session.get(url, timeout=settings.DEFAULT_REQUEST_TIMEOUT_SECONDS or 15) as response:
                    ...
                    logger.info(f"Successfully scraped and preprocessed content from {url} (length: {len(processed_text)} chars).")
                    return processed_text
            except aiohttp.ClientResponseError as e:
                logger.error(f"HTTP error while scraping {url}: {e.status} {e.message}", exc_info=False)
                return None
    ...
    async def ingest_content_from_url(self, monitored_source_id: int, url: str) -> Optional[Tuple[ScrapedContent, ContentEmbedding]]:
        ...
        """
        logger.info(f"Starting ingestion process for URL ID {monitored_source_id}: {url}")
        # 1. Scrape content
        scraped_text = await self._scrape_website_content(url)
        if scraped_text is None:
            logger.warning(f"Failed to scrape content from {url} (or timed out/error). Skipping ingestion.")
        ...
        if scraped_text == "ROBOTS_DISALLOWED":
            ...
            return None
        if not scraped_text.strip(): # Check if scraped text is empty after preprocessing
            logger.info(f"Scraped content from {url} is empty after preprocessing. Skipping ingestion.")
            return None
        ...
        try:
            logger.debug(f"Generating embeddings for content from {url} (length: {len(processed_text_for_embedding)} chars).")
            embeddings_list = await self.ai_model.generate_embeddings([processed_text_for_embedding])
            if not embeddings_list or not embeddings_list[0]:
                logger.error(f"Failed to generate embeddings for {url}. AI model returned no/empty embeddings.")
                return None
            embedding_vector = embeddings_list[0]
            logger.info(f"Successfully generated embeddings for {url}.")
        except NotImplementedError:
            logger.error(f"generate_embeddings not implemented by the AI model: {getattr(self.ai_model, 'model_name', type(self.ai_model).__name__)} for {url}")
            return None
        except Exception as e:
            logger.error(f"Error generating embeddings for {url}: {type(e).__name__} {e}", exc_info=True)
            return None
        ...
        try:
            ...
            logger.info(f"Successfully ingested and stored content (ID: {db_scraped_content.id}) and embedding (ID: {db_content_embedding.id}) for {url}")
            return db_scraped_content, db_content_embedding
        except Exception as e:
            self.db.rollback()
            logger.error(f"Database error storing content/embeddings for {url}: {type(e).__name__} {e}", exc_info=True)
            return None
    ...
    async def ingest_batch(self, sources: List[Tuple[int, str]]) -> List[Optional[Tuple[ScrapedContent, ContentEmbedding]]]:
        ...
        results = await asyncio.gather(*tasks, return_exceptions=True) # Handle exceptions from individual tasks
        ...
        for i, res in enumerate(results):
            ...
            if isinstance(res, Exception):
                logger.error(f"Task for ingesting {url} (ID: {source_id}) failed in batch: {type(res).__name__} {res}", exc_info=False) # exc_info already logged by individual task if it was an exception there
                processed_results.append(None)
            elif res is None:
                # This means ingest_content_from_url returned None (e.g. scrape failed, robots, no embedding)
                logger.warning(f"Task for ingesting {url} (ID: {source_id}) returned None in batch.")
                processed_results.append(None)
            else:
                success_count +=1
                processed_results.append(res)
        logger.info(f"Batch ingestion completed. {success_count}/{len(sources)} sources successfully ingested.")
