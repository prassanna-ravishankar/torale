...
class MonitoredSource(Base):
    ...
    name = Column(String, nullable=True, index=True) # User-defined name or query description
    ...
    last_significant_change_at = Column(DateTime(timezone=True), nullable=True) # When last sig. change was detected
    ...
    keywords_json = Column(Text, nullable=True) # Storing keywords as a JSON string list
    config_json = Column(Text, nullable=True)   # Storing other config like similarity_threshold as JSON string 