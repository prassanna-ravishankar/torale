-- Update existing migration to work with Python backend instead of Edge Functions
-- This replaces the HTTP-based trigger with a simple flag-based system

-- Remove the old trigger that tries to make HTTP calls
DROP TRIGGER IF EXISTS on_change_alert_created ON change_alerts;
DROP FUNCTION IF EXISTS trigger_change_alert_notification();

-- Create a simple trigger that just marks alerts for processing
