...
import { tasksApi, executionsApi, parseCron } from "../../lib/mockApi";
...
export const TaskDetail: React.FC<TaskDetailProps> = ({
  ...
  const loadData = async () => {
    ...
    try {
      const [taskData, executionsData, notificationsData] = await Promise.all([
        ...
        executionsApi.getExecutions(taskId),
        executionsApi.getNotifications(taskId),
      ]);
      setTask(taskData);
  ...
  const handleToggle = async () => {
    if (!task) return;
    try {
      await tasksApi.updateTask(taskId, { is_active: !task.is_active });
      await loadData();
    } catch (error) {
